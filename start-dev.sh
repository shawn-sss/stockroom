...
if ! command -v npm >/dev/null 2>&1; then
  echo "npm not found. Install Node.js (recommended via nvm) then try again."
  exit 1
fi

run_in_terminal "Backend" "cd \"$ROOT/backend\" && \"$PY\" -m uvicorn main:app --reload"
run_in_terminal "Frontend" "cd \"$ROOT/frontend\" && npm run dev"

if command -v code >/dev/null 2>&1; then
  code -r "$ROOT" >/dev/null 2>&1 &
fi

echo "Dev started: backend on http://127.0.0.1:8000 , frontend on http://localhost:5173"
